...
type State = Project & { selectedPartId?: string; editingPartId?: string; selectedMeshVertexIndex?: number; isPlaying: boolean; currentTime: number; editVertices: Record<string, Vec2[]>; savedKeyform?: { partId: string; value: 0 | 1 }; addParts(parts: Part[]): void; selectPart(id: string): void; beginMeshEdit(id: string): void; endMeshEdit(): void; selectMeshVertex(index?: number): void; updatePartMesh(id: string, mesh: MeshData, selectedMeshVertexIndex?: number): void; toggleVisible(id: string): void; movePart(id: string, dir: -1 | 1): void; setParameter(id: string, value: number): void; saveKeyform(value: 0 | 1): void; updatePartVertices(id: string, vertices: Vec2[]): void; play(): void; stop(): void; setTime(t: number): void }
export const useEditorStore = create<State>((set, get) => ({
  parts: [], parameters, parameterValues: { [PARAM_DEFORM_ID]: 0 }, animation, selectedPartId: undefined, editingPartId: undefined, selectedMeshVertexIndex: undefined, isPlaying: false, currentTime: 0, editVertices: {}, savedKeyform: undefined,
  addParts: parts => set(s => ({ parts: [...s.parts, ...parts], selectedPartId: parts.at(-1)?.id ?? s.selectedPartId, editingPartId: undefined, selectedMeshVertexIndex: undefined })),
  selectPart: id => set({ selectedPartId: id, editingPartId: undefined, selectedMeshVertexIndex: undefined }),
  beginMeshEdit: id => set({ selectedPartId: id, editingPartId: id, selectedMeshVertexIndex: undefined, isPlaying: false, editVertices: {}, savedKeyform: undefined }),
